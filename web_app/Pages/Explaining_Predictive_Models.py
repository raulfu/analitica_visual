...
if option == "Global":
    ...
    if model_selected == "Logistic (Simple - Who Wins?)":
        ...
        st.write(f"#### The average prediction for all games (target is if local team wins) is: {average_prediction:.4f}")
    ...
    selected_game_id_slider = st.selectbox("Select a Patient Index", X_test_games_id['game_id'])
    selected_game = df_header[df_header['game_id'] == selected_game_id_slider].iloc[0]
    st.write(f"#### {selected_game['team_a']} {selected_game['score_a']} vs {selected_game['score_b']} {selected_game['team_b']} ({selected_game['phase']} GAME ON {selected_game['season_code']} SEASON ON {selected_game['date']})")
