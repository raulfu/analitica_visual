...
df_total = df.groupby('team_name')[['points', 'three_points_made', 'total_rebounds', 'assists', 'steals', 'blocks_favour', 'valuation']].sum()

#excluding 2024
if selected_year == 'Historical (2007-2024)':
    df = df[df['season_code'] != 'E2024']
df_avg = df.groupby('team_name')[['points_per_game', 'three_points_made_per_game', 'total_rebounds_per_game', 'assists_per_game', 'steals_per_game', 'blocks_favour_per_game', 'valuation_per_game']].mean()
...
with col1:
    ...
    fig1 = create_plot(top_10_totals, f"Top 10 Teams by {selected_category} (Totals)", "Team", "Total", color = 'plum')
    st.pyplot(fig1)
    

# Plot Per-Game Chart
with col2:
    st.subheader(f"Top 10 Teams by {selected_category} (Per Game)")
    fig2 = create_plot(top_10_per_game, f"Top 10 Teams by {selected_category} (Per Game)", "Team", "Per Game", color = 'mediumpurple')
    st.pyplot(fig2)
...
# Plot Total Chart
with col1:
    st.subheader(f"Top 10 Teams by {selected_category} (History Totals)")
